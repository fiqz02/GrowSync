...
const EnhancedIrrigationControl = () => {
  ...
  const [servoState, setServoState] = useState<{ angle: number; timer: number }>({ angle: 0, timer: 0 });
  ...
  const startIrrigation = () => {
    ...
    if (isNaN(timerInMilliseconds) || timerInMilliseconds <= 0) {
      Alert.alert('Invalid Timer', 'Please enter a valid timer value greater than 0.');
      return;
    }
    ...
    set(servoRef, { angle: selectedAngle, timer: timerInMilliseconds })
      .then(() => {
        console.log(`Servo started at ${selectedAngle} degrees for ${timer} minutes`);
