...
const GraphWithDropdown = () => {
    ...
    const [analysis, setAnalysis] = useState({ highest: 0, lowest: 0, average: 0, date: '' });
        ...
        const listener = onValue(
            ...
            (snapshot) => {
                ...
                if (data) {
                    ...
                    const values: number[] = entries.map(([_, value]) => value as number);
                    ...
                    for (let i = 0; i < entries.length; i += 5) {
                        ...
                        const average =
                            chunk.reduce((sum, [_, value]) => sum + (value as number), 0) / chunk.length;
                    ...
                    setAnalysis({ highest, lowest, average: parseFloat(average.toFixed(2)), date });
                }
            },
            (error) => {
    ...
    return (
        <LinearGradient colors={['#87CEEB', '#4682B4']} style={styles.container}>
            ...
            <Text style={styles.graphTitle}>{selectedParameter === 'temperature' ? 'Temperature vs Time' : 'Humidity vs Time'}</Text>

            {graphData.length > 0 ? (
                <LineChart
                    data={{
                        labels: timestamps.map((t, index) => (index % 2 === 0 ? t : '')),
                        datasets: [
                            {
                                data: graphData,
                            },
                    ...
                    height={300}
                    yAxisSuffix={selectedParameter === 'temperature' ? '°C' : '%'}
                    chartConfig={{
            ...
            <View style={styles.analysisContainer}>
                <Text style={styles.analysisTitle}>{selectedParameter === 'temperature' ? 'Temperature' : 'Humidity'} Data</Text>
